  ...
  return (
    <div className="glass-card p-6 hover:shadow-lg transition-all group">
      ...
      <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300 mb-4">
        <div className="flex items-center gap-2"><MapPin className="w-4 h-4" /> {worker.location || 'Unknown'}</div>
        <div className="flex items-center gap-2"><Clock className="w-4 h-4" /> Rs. {worker.hourlyRate}/hr</div>
      ...
      <div className="flex gap-2">
        <button onClick={() => onHire(worker)} className="btn-primary flex-1 text-sm">Hire</button>
        <button className="glass-card px-3 py-2 text-sm">Details</button>
      </div>
    </div>
  );
}